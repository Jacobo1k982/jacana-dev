        ...
        <div ref={containerRef} className="relative z-50">
            <nav
                className={`fixed top-0 w-full transition-all duration-500 ease-in-out ${scrolled
                        ? "bg-black/80 backdrop-blur-xl shadow-[0_8px_32px_-4px_rgba(0,0,0,0.5)] border-b border-gray-800/60"
                        : "bg-gradient-to-r from-black/30 via-gray-900/30 to-black/30 backdrop-blur-2xl"
                    }`}
            >
                    ...
                    <div className="h-16 flex items-center justify-between">
                                    ...
                                    {item.subItems ? (
                                        <>
                                            <button className="relative px-3 py-2 text-sm font-medium text-gray-300 rounded-xl transition-all duration-300 group-hover:text-white">
                                                ...
                                                <span className="absolute -inset-2 bg-gradient-to-r from-indigo-500/40 to-purple-500/40 blur rounded-xl opacity-0 group-hover:opacity-70 transition-opacity -z-10"></span>
                                            </button>
                                            {/* Submenú */}
                                            <div className="absolute left-1/2 transform -translate-x-1/2 mt-4 w-[480px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                                                <div className="relative">
                                                    ...
                                                    <div className="relative bg-gray-900/90 backdrop-blur-2xl border border-gray-800/60 rounded-2xl shadow-2xl overflow-hidden z-10">
                                                        <div className="p-5 grid grid-cols-2 gap-4">
                                                            {item.subItems?.map((subItem) => (
                                                                <Link
                                                                    key={subItem.label}
                                                                    href={subItem.href}
                                                                    className="flex items-center p-3 rounded-xl bg-gray-800/40 hover:bg-gray-800/70 transition-all duration-300 group/sub border border-transparent hover:border-indigo-500/40"
                                        ...
                                        <Link
                                            href={item.href}
                                            className="relative px-3 py-2 text-sm font-medium text-gray-300 rounded-xl transition-all duration-300 hover:text-white"
                                            onClick={() => setIsOpen(false)}
                                        >
                                            {item.label}
                                        </Link>
                        ...
                        <div className="hidden lg:flex items-center space-x-3">
                            ...
                            >
                                ...
                                <span className="absolute inset-0 bg-gradient-to-r from-cyan-500 via-indigo-500 to-purple-600"></span>
