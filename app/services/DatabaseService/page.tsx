        ...
        >
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between py-4 text-left">
                <div className="flex items-center gap-3">
                    <div className="w-7 h-7 flex items-center justify-center border border-lime-400/30 shrink-0">
                        <Icon className="w-3.5 h-3.5 text-lime-400/70" />
                    </div>
                    <div>
                ...
                {serviceData.technologies.map((tech, i) => (
                    ...
                    >
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-[10px] text-lime-400/80">{tech.level}</span>
                        </div>
                        <div className="flex gap-0.5 mb-1.5">
                            {[...Array(10)].map((_, j) => (
                                <motion.div key={j} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 + j * 0.02 }}
                                    className={`h-0.5 flex-1 ${j < Math.round(tech.level / 10) ? 'bg-lime-400/70' : 'bg-slate-800'}`}
                                />
                            ))}
                        </div>
                ...
                {serviceData.process.map((step, i) => (
                    ...
                    >
                        <div className="absolute -left-[9px] top-0 w-[18px] h-[18px] flex items-center justify-center border border-slate-700/60 bg-[#080810] group-hover:border-lime-400/40 transition-colors">
                            <span className="text-[9px] font-medium text-slate-600 group-hover:text-lime-400/70 transition-colors">{step.step}</span>
                        </div>
                        <div>
                    ...
                    >
                        <div className="border-l-2 border-lime-400/30 pl-4">
                            ...
                            <div className="flex items-center gap-2">
                                <div className="w-6 h-6 flex items-center justify-center border border-lime-400/30 text-[8px] text-lime-400/80">
        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    >
                        <div className="h-px bg-lime-400/60 shrink-0" />
                        <div className="absolute inset-0 opacity-[0.025] pointer-events-none"
                            style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)'/%3E%3C/svg%3E")`, backgroundSize: '128px 128px' }}
                        />
                        <div className="absolute top-1/4 right-0 w-[400px] h-[400px] rounded-full bg-lime-900/8 blur-[140px] pointer-events-none" />
                        ...
                        <div className="relative px-8 pt-8 pb-6 border-b border-slate-800/60 shrink-0">
                            ...
                            <div className="flex items-start gap-4 pr-12">
                                <div className="w-11 h-11 flex items-center justify-center border border-lime-400/30 shrink-0">
                                    <Database className="w-5 h-5 text-lime-400/80" />
                                </div>
                                <div>
                                ...
                                <div className="grid grid-cols-3 gap-px bg-slate-800/40">
                                    {[
                                        ...
                                        <div key={s.label} className="bg-[#080810] px-3 py-4 text-center">
                                            <p className="text-2xl font-light text-lime-400/80 leading-none mb-1"
                                                style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}
                                            >{s.value}</p>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {serviceData.benefits.map(b => {
                                            const Icon = b.icon;
                                            return (
                                                <div key={b.title} className="flex items-center gap-2 px-3 py-2 border border-lime-400/20 text-lime-400/70 text-xs">
                        ...
                        <div className="h-px bg-lime-400/20 shrink-0" />
                    </motion.div>
                </>
            )}
        </AnimatePresence>
...
function MainCard({ onClick }: { onClick: () => void }) {
    return (
        ...
        >
            <div className="absolute top-0 left-0 right-0 h-px bg-lime-400/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            <div className="absolute inset-0 opacity-[0.025] pointer-events-none"
                style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)'/%3E%3C/svg%3E")`, backgroundSize: '128px 128px' }}
            />
            <div className="absolute top-0 right-0 w-[300px] h-[300px] rounded-full bg-lime-900/8 blur-[100px] pointer-events-none" />

            <div className="relative px-8 py-8">
                <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-4">
                        <div className="w-11 h-11 flex items-center justify-center border border-lime-400/30 group-hover:border-lime-400/50 transition-colors">
                            <Database className="w-5 h-5 text-lime-400/80" />
                        </div>
                        <div>
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-600 group-hover:text-lime-400/60 transition-colors">
                ...
                <div className="flex flex-wrap gap-1.5 mb-6">
                    {['PostgreSQL', 'MongoDB', 'Redis', 'MySQL', 'Prisma'].map(tech => (
                        <span key={tech} className="px-2.5 py-1 text-[10px] uppercase tracking-[0.1em] border border-lime-400/20 text-lime-400/60">
                            {tech}
                        </span>
                    ))}
                </div>

                <div className="h-px bg-gradient-to-r from-slate-800/80 via-lime-400/20 to-transparent mb-6" />
    ...
    return (
        <div className="min-h-screen bg-[#080810] flex items-center justify-center p-8">
            <div className="fixed inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full bg-lime-900/8 blur-[140px]" />
