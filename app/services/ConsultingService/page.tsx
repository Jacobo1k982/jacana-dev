...
import {
    ...
    ChevronRight, X, Sparkles, Target, Settings,
    Award, Briefcase, ArrowRight, ExternalLink, Rocket
} from 'lucide-react';
...
const serviceData = {
    ...
    features: [
        ...
        { icon: FileSearch, title: 'Auditoría de Código', desc: 'Análisis profundo de calidad y seguridad', detail: 'Revisamos tu codebase identificando debt técnico, vulnerabilidades de seguridad y oportunidades de optimización. Entregamos reportes con priorización.' },
    ...
    technologies: [
        { name: 'Arquitectura', level: 95, description: 'Diseño de sistemas complejos' },
        { name: 'Code Review', level: 95, description: 'Análisis de calidad de código' },
        { name: 'Performance', level: 92, description: 'Optimización y tuning' },
        { name: 'Seguridad', level: 88, description: 'Auditoría y pentesting' },
        { name: 'DevOps', level: 90, description: 'CI/CD y automatización' },
        { name: 'Scrum/Agile', level: 95, description: 'Metodologías ágiles' },
    ],
    benefits: [
    ...
    testimonials: [
        { name: 'Patricia Rojas', role: 'CEO @ TechStartup', text: 'Su consultoría nos ahorró $500K en decisiones tecnológicas equivocadas. Invalorable.' },
        { name: 'Miguel Torres', role: 'CTO @ Enterprise Co.', text: 'La auditoría de arquitectura transformó nuestra forma de desarrollar software.' },
        { name: 'Laura Sánchez', role: 'Tech Lead @ Fintech', text: 'Los workshops de arquitectura elevaron el nivel técnico de todo el equipo.' },
    ],
    funFacts: [
        ...
        >
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between py-4 text-left">
                <div className="flex items-center gap-3">
                    <div className="w-7 h-7 flex items-center justify-center border border-violet-400/30 shrink-0">
                ...
                {serviceData.technologies.map((tech, i) => (
                    ...
                    >
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-[10px] text-violet-400/80">{tech.level}</span>
                        </div>
                        <div className="flex gap-0.5 mb-1.5">
                            {[...Array(10)].map((_, j) => (
                                <motion.div key={j} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.05 + j * 0.02 }}
                                    className={`h-0.5 flex-1 ${j < Math.round(tech.level / 10) ? 'bg-violet-400/70' : 'bg-slate-800'}`}
                                />
                            ))}
                        </div>
        ...
        process: (
            <div className="border-l border-slate-800/60 ml-3 space-y-0">
                {serviceData.process.map((step, i) => (
                    ...
                    >
                        <div className="absolute -left-[9px] top-0 w-[18px] h-[18px] flex items-center justify-center border border-slate-700/60 bg-[#080810] group-hover:border-violet-400/40 transition-colors">
                            <span className="text-[9px] font-medium text-slate-600 group-hover:text-violet-400/70 transition-colors">{step.step}</span>
                        </div>
                        <div>
        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    >
                        <div className="h-px bg-violet-400/60 shrink-0" />
                        <div className="absolute inset-0 opacity-[0.025] pointer-events-none"
                            style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)'/%3E%3C/svg%3E")`, backgroundSize: '128px 128px' }}
                        />
                        <div className="absolute top-1/4 right-0 w-[400px] h-[400px] rounded-full bg-violet-900/8 blur-[140px] pointer-events-none" />
                        ...
                        <div className="relative px-8 pt-8 pb-6 border-b border-slate-800/60 shrink-0">
                            ...
                            <div className="flex items-start gap-4 pr-12">
                                <div className="w-11 h-11 flex items-center justify-center border border-violet-400/30 shrink-0">
                            ...
                            <div className="px-8 py-8 space-y-10">
                                ...
                                <div className="grid grid-cols-3 gap-px bg-slate-800/40">
                                    {[
                                        ...
                                        { label: 'Horas', value: serviceData.stats.hours },
                                    ].map(s => (
                                        <div key={s.label} className="bg-[#080810] px-3 py-4 text-center">
                                            <p className="text-2xl font-light text-violet-400/80 leading-none mb-1"
                                                style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}
                                            >{s.value}</p>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {serviceData.benefits.map(b => {
                                            const Icon = b.icon;
                                            return (
                                                <div key={b.title} className="flex items-center gap-2 px-3 py-2 border border-violet-400/20 text-violet-400/70 text-xs">
                                    ...
                                    <div className="flex border border-slate-800/60 mb-8">
                                        {tabs.map(tab => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    {isActive && (
                                                        <motion.span layoutId="consultingTabIndicator"
                                                            className="absolute top-0 left-0 right-0 h-px bg-violet-400/50"
                        ...
                        <div className="h-px bg-violet-400/20 shrink-0" />
                    </motion.div>
                </>
            )}
        </AnimatePresence>
...
function MainCard({ onClick }: { onClick: () => void }) {
    return (
        ...
        >
            <div className="absolute top-0 left-0 right-0 h-px bg-violet-400/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            <div className="absolute inset-0 opacity-[0.025] pointer-events-none"
                style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)'/%3E%3C/svg%3E")`, backgroundSize: '128px 128px' }}
            />
            <div className="absolute top-0 right-0 w-[300px] h-[300px] rounded-full bg-violet-900/8 blur-[100px] pointer-events-none" />

            <div className="relative px-8 py-8">
                <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-4">
                        <div className="w-11 h-11 flex items-center justify-center border border-violet-400/30 group-hover:border-violet-400/50 transition-colors">
                            <Users className="w-5 h-5 text-violet-400/80" />
                        </div>
                        <div>
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-600 group-hover:text-violet-400/60 transition-colors">
                ...
                <div className="flex flex-wrap gap-1.5 mb-6">
                    {['Arquitectura', 'Auditoría', 'DevOps', 'Agile', 'Cloud'].map(tech => (
                        <span key={tech} className="px-2.5 py-1 text-[10px] uppercase tracking-[0.1em] border border-violet-400/20 text-violet-400/60">
                            {tech}
                        </span>
                    ))}
                </div>

                <div className="h-px bg-gradient-to-r from-slate-800/80 via-violet-400/20 to-transparent mb-6" />

                <div className="grid grid-cols-3 gap-px bg-slate-800/40">
                    {[
                        { value: `${serviceData.stats.projects}+`, label: 'Proyectos' },
                        { value: `${serviceData.stats.satisfaction}%`, label: 'Satisfacción' },
                        { value: serviceData.stats.hours, label: 'Horas' },
                    ].map(s => (
    ...
    return (
        <div className="min-h-screen bg-[#080810] flex items-center justify-center p-8">
            <div className="fixed inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] rounded-full bg-violet-900/8 blur-[140px]" />
