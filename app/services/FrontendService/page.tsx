...
import {
    ...
    ChevronRight, X, CheckCircle, Sparkles, Layers,
    Palette, Globe, Maximize, ArrowRight, Star,
    Play, Pause, ExternalLink, Heart, Share2,
    BookOpen, Lightbulb, Rocket, Target, Users,
    TrendingUp, Award, Clock, MousePointer, Move3d
} from 'lucide-react';
...
const serviceData = {
    ...
    technologies: [
        { name: 'React', level: 95, icon: '⚛️', color: 'from-cyan-400 to-blue-500', description: 'Biblioteca líder para interfaces interactivas' },
        { name: 'Next.js', level: 92, icon: '▲', color: 'from-gray-400 to-gray-600', description: 'Framework React con SSR y optimizaciones' },
        { name: 'Vue.js', level: 88, icon: '💚', color: 'from-emerald-400 to-green-500', description: 'Framework progresivo y versátil' },
        { name: 'Angular', level: 85, icon: '🅰️', color: 'from-red-400 to-rose-500', description: 'Platform completo para apps empresariales' },
        { name: 'TypeScript', level: 95, icon: '📘', color: 'from-blue-400 to-indigo-500', description: 'JavaScript con tipado estático' },
        { name: 'Tailwind CSS', level: 93, icon: '🎨', color: 'from-teal-400 to-cyan-500', description: 'Utility-first CSS framework' }
    ],
    benefits: [
    ...
    testimonials: [
        { name: 'María García', role: 'CTO @ TechStartup', text: 'El equipo transformó nuestra visión en una interfaz impresionante. El rendimiento mejoró un 300%.', avatar: '👩‍💼' },
        { name: 'Carlos Ruiz', role: 'Founder @ E-commerce', text: 'Increíble atención al detalle. Nuestra tasa de conversión aumentó un 45% después del rediseño.', avatar: '👨‍💻' }
    ],
    funFacts: [
...
function FloatingOrb({ delay = 0, size = 'md', color = 'cyan' }: { delay?: number; size?: 'sm' | 'md' | 'lg'; color?: string }) {
    const sizeClasses = {
        sm: 'w-2 h-2',
        md: 'w-3 h-3',
        lg: 'w-4 h-4'
    ...
    return (
        <motion.div
            ...
            style={{
                background: `radial-gradient(circle, ${color === 'cyan' ? 'rgba(6, 182, 212, 0.8)' : 'rgba(168, 85, 247, 0.8)'} 0%, transparent 70%)`,
                filter: 'blur(1px)'
            }}
            initial={{
                x: position.x,
...
function EnhancedParticles() {
    ...
    const [particles] = useState(() =>
        [...Array(30)].map((_, i) => ({
            ...
            delay: Math.random() * 5,
            colorClass: i % 3 === 0 ? 'bg-cyan-400/20' : i % 3 === 1 ? 'bg-blue-400/15' : 'bg-purple-400/10'
        }))
    );
...
function EnhancedCodeRain() {
    const codeSnippets = [
        { code: '<div>', color: 'text-pink-400' },
        { code: 'className=', color: 'text-cyan-400' },
        { code: 'return (', color: 'text-purple-400' },
        { code: 'export default', color: 'text-blue-400' },
        { code: 'import { }', color: 'text-yellow-400' },
        { code: 'const [state]', color: 'text-orange-400' },
        { code: 'useState()', color: 'text-green-400' },
        { code: 'useEffect()', color: 'text-indigo-400' },
        { code: '{...props}', color: 'text-rose-400' },
        { code: '.map()', color: 'text-teal-400' },
        { code: '</>', color: 'text-pink-400' },
        { code: 'async/await', color: 'text-amber-400' }
    ];
...
function TechCard({ tech, index, onClick }: { tech: typeof serviceData.technologies[0]; index: number; onClick: () => void }) {
    ...
    return (
        ...
        >
            <div className="relative p-5 rounded-2xl bg-gradient-to-br from-white/[0.08] to-white/[0.02] border border-white/10 hover:border-cyan-500/40 transition-all duration-300 backdrop-blur-sm hover:shadow-xl hover:shadow-cyan-500/10">
                {/* Glow effect */}
            ...
            <motion.div
                ...
                className={`relative p-5 rounded-2xl border transition-all duration-500 cursor-pointer overflow-hidden
                    ${isExpanded
                        ...
                        : 'bg-white/[0.02] border-white/5 hover:border-cyan-500/30 hover:bg-cyan-500/5'
                    }`}
            >
                ...
                <div className="relative z-10">
                    <div className="flex items-start gap-4">
                        <motion.div
                            ...
                            className={`p-3 rounded-xl transition-colors ${isExpanded ? 'bg-cyan-500/20' : 'bg-white/5 group-hover:bg-cyan-500/10'}`}
                        >
                            <feature.icon className={`w-5 h-5 transition-colors ${isExpanded ? 'text-cyan-300' : 'text-cyan-400'}`} />
                        </motion.div>
                        <div className="flex-1">
                            <div className="flex items-center justify-between">
                                <h4 className={`font-semibold transition-colors ${isExpanded ? 'text-cyan-300' : 'text-white group-hover:text-cyan-300'}`}>
                                    {feature.title}
                                </h4>
                                <motion.div
...
function ProcessTimeline() {
    return (
        <div className="relative">
            {/* Vertical line */}
            <div className="absolute left-4 top-0 bottom-0 w-px bg-gradient-to-b from-cyan-500/50 via-blue-500/30 to-purple-500/50" />
            ...
            <div className="space-y-6">
                {serviceData.process.map((step, index) => (
                    ...
                    >
                        {/* Step indicator */}
                        <div className="absolute left-0 w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center text-white text-xs font-bold shadow-lg shadow-cyan-500/30 group-hover:scale-110 transition-transform">
                            {step.step}
                        </div>

                        <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 group-hover:border-cyan-500/20 group-hover:bg-cyan-500/5 transition-all">
                            <h4 className="font-semibold text-white group-hover:text-cyan-300 transition-colors">{step.title}</h4>
                ...
                {serviceData.funFacts.map((_, index) => (
                    <motion.div
                        ...
                        animate={{
                            ...
                            backgroundColor: currentIndex === index ? 'rgb(6, 182, 212)' : 'rgba(255,255,255,0.2)'
                        }}
...
function TestimonialCard({ testimonial, index }: { testimonial: typeof serviceData.testimonials[0]; index: number }) {
    return (
        <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: index * 0.1 }}
            className="p-6 rounded-2xl bg-gradient-to-br from-white/[0.05] to-white/[0.02] border border-white/10 hover:border-cyan-500/30 transition-all"
        >
            <div className="flex items-start gap-4">
...
function AnimatedStats() {
    ...
    const clientsCounter = useAnimatedCounter(serviceData.stats.clients);
    ...
    return (
        <div className="grid grid-cols-3 gap-4">
            {[
                { label: 'Proyectos', value: projectsCounter.count, suffix: '+', icon: Globe },
                { label: 'Satisfacción', value: satisfactionCounter.count, suffix: '%', icon: Award },
                { label: 'Clientes', value: clientsCounter.count, suffix: '+', icon: Users }
            ].map((stat, index) => (
                <motion.div
                    key={stat.label}
                    ...
                    transition={{ delay: index * 0.1 }}
                    className="group p-5 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-cyan-500/30 transition-all cursor-pointer hover:bg-cyan-500/5"
                >
                    <stat.icon className="w-5 h-5 text-cyan-400 mb-3 group-hover:scale-110 transition-transform" />
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed inset-0 md:inset-4 lg:inset-8 z-50 overflow-hidden rounded-3xl border border-cyan-500/20 bg-gradient-to-br from-[#0a0a20] via-[#0d1020] to-[#0a0a20] shadow-2xl"
                    >
                        ...
                        <div className="absolute inset-0 overflow-hidden">
                            ...
                            <ClientOnly>
                                <EnhancedCodeRain />
                                {[...Array(6)].map((_, i) => (
                                    <FloatingOrb key={i} delay={i * 0.5} size={i % 2 === 0 ? 'md' : 'sm'} color={i % 2 === 0 ? 'cyan' : 'purple'} />
                                ))}
                            </ClientOnly>
                        ...
                        <div className="relative z-10 h-full overflow-y-auto custom-scrollbar">
                            ...
                            <div className="relative h-72 md:h-80 overflow-hidden">
                                <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/30 via-blue-500/20 to-purple-500/20" />
                                <div className="absolute inset-0 bg-gradient-to-t from-[#0a0a20] via-[#0a0a20]/50 to-transparent" />

                                {/* Animated shapes */}
                                <motion.div
                                    animate={{ rotate: 360 }}
                                    transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}
                                    className="absolute top-10 right-10 w-32 h-32 border border-cyan-500/20 rounded-3xl"
                                />
                                <motion.div
                                    animate={{ rotate: -360 }}
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
                                    ...
                                    >
                                        <motion.div
                                            ...
                                            className="p-5 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 backdrop-blur-sm shadow-xl shadow-cyan-500/20"
                                        >
                                        ...
                                        <div>
                                            <motion.h2
                                                ...
                                                className="text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white via-cyan-200 to-blue-300"
                                            >
                                                {serviceData.title}
                                            </motion.h2>
                                            <motion.p
                                ...
                                >
                                    {serviceData.benefits.map((benefit, index) => (
                                        <motion.div
                                            ...
                                            className="flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 text-cyan-300 text-sm cursor-pointer hover:border-cyan-500/40 transition-colors"
                                        >
                                ...
                                >
                                    <div className="flex flex-col sm:flex-row gap-4">
                                        <motion.button
                                            ...
                                            whileTap={{ scale: 0.98 }}
                                            className="flex-1 px-8 py-4 rounded-xl bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-500 text-white font-semibold flex items-center justify-center gap-3 shadow-xl relative overflow-hidden group"
                                        >
                                            <motion.div
                                                animate={{ x: ['-100%', '200%'] }}
                                                transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}
                                        ...
                                        <motion.button
                                            ...
                                            whileTap={{ scale: 0.98 }}
                                            className="px-8 py-4 rounded-xl bg-white/5 border border-white/10 text-cyan-400 font-semibold hover:bg-cyan-500/10 transition-colors flex items-center justify-center gap-2"
                                        >
...
function MainCard({ onClick }: { onClick: () => void }) {
    ...
    return (
        ...
        >
            {/* Card container */}
            <div className="relative rounded-3xl border border-cyan-500/20 bg-gradient-to-br from-[#0a0a20] to-[#0d1020] overflow-hidden transition-all duration-500 hover:border-cyan-500/40 hover:shadow-2xl hover:shadow-cyan-500/20">
                {/* Animated background */}
                <div className="absolute inset-0 overflow-hidden">
                    <motion.div
                        animate={{
                            background: isHovered
                                ? 'radial-gradient(circle at 50% 50%, rgba(6, 182, 212, 0.2) 0%, rgba(168, 85, 247, 0.1) 50%, transparent 70%)'
                                : 'radial-gradient(circle at 50% 50%, rgba(6, 182, 212, 0) 0%, transparent 50%)'
                        }}
                    ...
                    <motion.div
                        ...
                        className="absolute -top-20 -right-20 w-40 h-40 bg-cyan-500 rounded-full blur-3xl"
                    />
                    <motion.div
                ...
                <div className="relative z-10 p-8">
                    ...
                    <div className="flex items-start justify-between mb-6">
                        <motion.div
                            ...
                            transition={{ duration: 0.5 }}
                            className="p-4 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 backdrop-blur-sm shadow-xl shadow-cyan-500/10"
                        >
                        ...
                        <motion.div
                            ...
                            className="flex items-center gap-1 text-cyan-400 text-sm font-medium bg-cyan-500/10 px-3 py-1.5 rounded-full border border-cyan-500/20"
                        >
                    ...
                    <motion.h3
                        ...
                        style={{
                            backgroundImage: 'linear-gradient(90deg, #fff, #22d3ee, #818cf8, #fff)',
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        {['React', 'Next.js', 'Vue.js', 'TypeScript', 'Tailwind'].map((tech, i) => (
                            <motion.span
                                key={tech}
                                ...
                                className="px-3 py-1.5 text-xs rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 cursor-default"
                            >
                                {tech}
                            </motion.span>
                        ))}
                    ...
                    <div className="grid grid-cols-3 gap-3 pt-5 border-t border-white/5">
                        {[
                            { icon: Globe, value: `${serviceData.stats.projects}+`, label: 'Proyectos' },
                            { icon: Award, value: `${serviceData.stats.satisfaction}%`, label: 'Satisfacción' },
                            { icon: Users, value: `${serviceData.stats.clients}+`, label: 'Clientes' }
                        ].map((stat, i) => (
                            <motion.div
                                key={stat.label}
                                initial={{ opacity: 0, y: 10 }}
                                ...
                                className="flex flex-col items-center p-3 rounded-xl bg-white/[0.02] border border-white/5 hover:border-cyan-500/20 transition-all group/stat"
                            >
                                <stat.icon className="w-4 h-4 text-cyan-400 mb-1 group-hover/stat:scale-110 transition-transform" />
                ...
                <motion.div
                    animate={{ opacity: isHovered ? 1 : 0 }}
                    className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-cyan-500/30 to-transparent blur-2xl"
                />
                <motion.div
                    animate={{ opacity: isHovered ? 1 : 0 }}
...
export default function FrontendService() {
    ...
    useEffect(() => {
        const styleId = 'frontend-service-custom-styles';
        ...
        style.textContent = `
            ...
            .custom-scrollbar::-webkit-scrollbar-thumb:hover {
                background: linear-gradient(180deg, rgba(6, 182, 212, 0.5), rgba(139, 92, 246, 0.5));
            }
