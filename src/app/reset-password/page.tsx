...
export default function ResetPassword() {
    ...
    // Actualizar la fortaleza de la contraseña cuando cambia el input
    useEffect(() => {
    ...
    const getStrengthColor = (index: number, currentStrength: number) => {
        if (index < currentStrength) {
            if (currentStrength <= 2) return "bg-[#f85149]"; // Red (Débil)
            if (currentStrength === 3) return "bg-[#d29922]"; // Naranja (Regular)
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        // Aseguramos que token y email se envíen aunque no estén en los inputs visibles
                ...
                <div className="rounded-md border border-[#30363d] bg-[#161b22] shadow-2xl p-8">
                    <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="mt-3 space-y-2">
                                    <div className="flex gap-1 h-1">
                                        {[0, 1, 2, 3, 4].map((index) => (
                                            <div
                                                key={index}
                                                className={`flex-1 rounded-full transition-colors duration-300 ${getStrengthColor(index, strength)}`}
                                            />
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="space-y-2">
                            ...
                            {/* Requisitos de contraseña (Corregido) */}
                            {password && (
