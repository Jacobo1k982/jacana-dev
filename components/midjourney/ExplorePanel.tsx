        ...
        <AnimatePresence>
            {isOpen && (
                <>
                    ...
                    >
                        {/* Top accent */}
                        <div className="h-px bg-gradient-to-r from-transparent via-lime-400/40 to-transparent shrink-0" />

                        {/* ── HEADER ── */}
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800/60 shrink-0">
                            <div>
                                <p className="text-[9px] uppercase tracking-[0.3em] text-lime-400/60 mb-0.5 flex items-center gap-2">
                                    <span className="w-1 h-1 rounded-full bg-lime-400/80 animate-pulse" />
                            ...
                            <button
                                onClick={onClose}
                                className="w-8 h-8 flex items-center justify-center border border-slate-700/60 hover:border-lime-400/40 text-slate-500 hover:text-slate-300 transition-all"
                            >
                                            ...
                                            >
                                                ...
                                                <div className="aspect-video relative overflow-hidden shrink-0">
                                                    ...
                                                    <div className="absolute top-2 left-2 flex items-center gap-1.5 px-2 py-1 bg-[#080810]/80 backdrop-blur-sm border border-slate-800/60">
                                                        <CatIcon className="w-3 h-3 text-lime-400/70" />
                                                ...
                                                <div className="flex flex-col flex-1 px-4 py-4">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <a
                                                            href={project.demo}
                                                            className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-white text-[#080810] hover:bg-lime-50 text-xs font-medium uppercase tracking-[0.1em] transition-colors"
                                                        >
                        ...
                        <div className="px-6 py-3 border-t border-slate-800/60 flex items-center justify-between shrink-0">
                            <span className="text-[10px] uppercase tracking-[0.2em] text-slate-600">
                                ...
                                {activeCat && activeCategory !== 'all' && (
                                    <span className="text-lime-400/50 ml-1">· {activeCat.label}</span>
                                )}
                            </span>
                            <button
                                onClick={onClose}
                                className="text-[10px] uppercase tracking-[0.2em] text-slate-600 hover:text-lime-400/70 transition-colors"
                            >
                                Cerrar
                            </button>
                        </div>
                        ...
                        {/* Bottom accent */}
                        <div className="h-px bg-gradient-to-r from-transparent via-lime-400/15 to-transparent shrink-0" />
                    </motion.div>
                </>
            )}
        </AnimatePresence>
